 ...
 * 
 */
package com.xgame.framework.schedule.core;
...
/**  
 * @Description:quantz的CronExpression的getTimeBefore(根据传入的时间获取前一次触发的时间)，但是没有实现，
 * CronExpressionEx覆盖了getTimeBefore方法，主要用于游戏中的停服，当机导致超过执行的时间，游戏服启动时，获取启动时刻前一次执行时间，当玩家身上保存的job执行时刻小于job的前一次的执行则进行相应的重置或者刷新操作。
...
//	private static Logger log = Logger.getLogger(CronExpressionEx.class);
	...
	/**
	 * @Description 获取当前时间之前的一次执行时间，模仿下一次执行时间计算
	 * 这里依然有一个隐患，就是如果停机时间特别长，在这段时间内，有多次定时任务的时间执行点，那么重启之后，也只能够找到最近的
	 * 前一次执行时间
	 * @param endTime 当前时间
	 * @return
	 * 
	 * @date 2015年9月23日 下午2:00:42
	 */
	@Override
	public Date getTimeBefore(Date endTime) {
		...
		/*
		 * 往前推一秒，cron最小精度为秒，可能碰巧当前时间就是job触发的时间，所以往前推一秒获取前一次触发的时间
		 */
		endTime = new Date(endTime.getTime() - TimeUtils.ONE_SECONDS_MS);
...
/*	public static void main(String[] args) throws FileNotFoundException, ParseException {
		...
		//3月1号比较特殊，往前推是2月，2月只有28或者29号，测试边界
		now.set(2014, 2, 1, 0, 0, 0);
		
		while (scanner.hasNextLine()) {
			CronExpressionEx cronExpression = new CronExpressionEx(scanner.nextLine());
			System.err.print("job开始时间:" + now.getTime().toLocaleString() + "\t");
			System.err.print("cron表达式:" + cronExpression + "\t");
			Date timeAfter = cronExpression.getTimeAfter(now.getTime());
			System.err.print("job下一步执行时间:" + (timeAfter == null ? "" : timeAfter.toLocaleString()) + "\t");
			Date timeBefore = cronExpression.getTimeBefore(now.getTime());
			System.err.println("job上一步执行时间:" + (timeBefore == null ? "" : timeBefore.toLocaleString()) + "\t");
		}
...
/*	public static void main(String[] args) throws ParseException {
		Calendar now = Calendar.getInstance();
		now.set(2014, 8, 12, 0, 0, 0);
		
//		CronExpressionEx cronExpression = new CronExpressionEx("0 15 10 ? * 6L");
//		CronExpressionEx cronExpression = new CronExpressionEx("0 15 10 ? * 6#3");
		CronExpressionEx cronExpression = new CronExpressionEx("0 0 0 LW * ?");
		System.err.print("job开始时间:" + now.getTime().toLocaleString() + "\t");
		System.err.print("cron表达式:" + cronExpression + "\t");
		Date timeAfter = cronExpression.getTimeAfter(now.getTime());
		System.err.print("job下一步执行时间:" + (timeAfter == null ? "" : timeAfter.toLocaleString()) + "\t");
