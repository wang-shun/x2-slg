...
public class ReqRemoveBanPlayerHandler extends PlayerCommand<ReqRemoveBanPlayerMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.removeBanPlayerId(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId);
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.removeBanPlayerId(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId);
			}
		}
    }
}
