 ...
 */
@Component
public class AllianceBattleTeamHandler extends AbstractFightHandler {
	...
	public Battle init(WarFightParam warFightParam) {
		...
		List<WorldMarchSoldier> marchSoldierList = new ArrayList<>();
		for(WorldMarch worldMarch :warFightParam.getAttackMarchList()) {
			marchSoldierList.add(worldMarch.getWorldMarchSoldier());
		}
		
		warAttacker.initSoldier(warFightParam.getAttackPlayer(), marchSoldierList, 0);
	...
	public void fightEnd(Battle battle, WarEndReport warEndReport) {
		...
		// 发送战报
		warAttacker.getPlayer().getPlayerMailInfoManager().sendTeamAttackEmailInfo(battle);
		...
		String result = JsonUtil.toJSON(battle.getResWarDataMessage());
		
		WarFightParam warFightParam = battle.getWarFightParam();
		warFightParam.getAttackWorldMarch().setAttach(result);
		
		if(resWarEndMessage != null && warEndReport.getWinUid() == warAttacker.getPlayer().getRoleId()) {
			...
			if(defendSpriteInfo != null && defendSpriteInfo.getParam() != null) {
				...
				if(defendSprite != null) {
					defendSprite.setAttackTime(TimeUtils.getCurrentTimeMillis());
			...
			// 队友返回
			AllianceBattleTeam allianceBattleTeam = InjectorUtil.getInjector().dbCacheService.get(AllianceBattleTeam.class, warFightParam.getTeamId());
			...
			for(WorldMarch worldMarch : warFightParam.getDefendMarchList()) {
				...
				if(worldMarch.executor instanceof ReinforceMarch && !((ReinforceMarch)worldMarch.executor).checkSameAlliancePlayer()) {
					worldMarch.getExecutor().handleReturn();
				}
			}
		}
		...
		// 解散队伍
		allianceBattleTeamManager.cancelAllianceBattleTeam(warFightParam.getTeamId());
