 ...
 */
@Slf4j
public class GateConnection2 extends Thread {
	...
	private ObjectFactory<GateMessageHandler2> serverHandler;

    public GateConnection2(String conName, String host, int port, long connectPeriod, EventLoopGroup eventLoop, int gateId, ObjectFactory<GateMessageHandler2> serverHandler) {
        this.conName = conName;
        this.host = host;
        this.port = port;
        this.connectPeriod = connectPeriod;
        this.serverHandler = serverHandler;
        this.eventLoopGroup = eventLoop;
    }
        ...
        while (working) {
            try {
                ...
                while (gateMessage != null) {
                    ...
                    log.info("反馈给客户端的消息，messageID:{}", gateMessage.getMsgId());
    ...
    public void sendKickSession(SessionShutDown sessionShutDown) {
    	if(channel !=  null) {
    		channel.writeAndFlush(sessionShutDown);
    	}
    }
            ...
            bootstrap.group(eventLoopGroup)
                    ...
                    .handler(new ChannelInitializer<NioSocketChannel>() {
                        ...
                        protected void initChannel(NioSocketChannel ch) throws Exception {
                        	ch.pipeline().addLast("lengthDecode", new LengthFieldBasedFrameDecoder(65535, 0, 4, 0, 4));
                            ch.pipeline().addLast(new LengthFieldPrepender(4));
                        	ch.pipeline().addLast(new GateCodec3());
