 ...
 */
@Component
public class EventListener31 extends BaseEventListener {
	
	public void onAction(IEventObject event) {
		DamageSideEventObject dsEvent = (DamageSideEventObject)event;
		//获取个人正在进行中的事件
	 ...
	 * @param scEvent
	 */
	private void updateScore(Map<Long,EventScoreInfo> eventScoreInfo,DamageSideEventObject dsEvent,String name,long playerOrAllianceId,Set<Long> playerIds){
			...
			for(Long id : eventScoreInfo.keySet()){
				...
				if(pEvent.getStartTime() <= TimeUtils.getCurrentTimeMillis()){
					...
					for(EventTaskPir etp : eventTaskMap.values()){
						//此处需要结合参数判断
						//判断事件类型 类型 士兵类型
						if(pEvent.getType()==etp.getEventType() && EventTaskTypeEnum.HURT_SOLDIER_TYPE.getValue() == etp.getType() && etp.getV1() != null && dsEvent.getDamageSideSoldier().get(Integer.parseInt(etp.getV1())) != null){
	...
	public int[] focusActions() {
		return new int[]{EventTypeConst.DAMAGE_SIDE};
	}
}
