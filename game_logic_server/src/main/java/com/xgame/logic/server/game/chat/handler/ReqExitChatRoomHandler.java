...
public class ReqExitChatRoomHandler extends PlayerCommand<ReqExitChatRoomMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.exitRoom(crossPlayer, Long.valueOf(message.roomKey));
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.exitRoom(crossPlayer, Long.valueOf(message.roomKey));
			}
		}
    }
}
