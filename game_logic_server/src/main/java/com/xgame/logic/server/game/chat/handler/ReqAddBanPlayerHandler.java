...
public class ReqAddBanPlayerHandler extends PlayerCommand<ReqAddBanPlayerMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.addBanPlayerId(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId);
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.addBanPlayerId(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId);
			}
		}
    }
}
