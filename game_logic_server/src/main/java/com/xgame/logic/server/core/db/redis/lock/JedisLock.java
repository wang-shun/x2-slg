package com.xgame.logic.server.core.db.redis.lock;
...
public class JedisLock {
	 ...
	 */
	public JedisLock(String lockKey, Jedis jedis) {
		this.jedis = jedis;
	...
	public synchronized boolean acquire(Jedis jedis) throws InterruptedException {
		...
		while (timeout >= 0) {
			...
			if (currentValueStr != null
					&& Long.parseLong(currentValueTimeStr) < System
							.currentTimeMillis()) {
	...
	public synchronized boolean acquire2(Jedis jedis) throws InterruptedException {
		int timeout = timeoutMsecs;
		while (timeout >= 0) {
			long expires = System.currentTimeMillis() + expireMsecs + 1;
			String expiresStr = String.valueOf(expires); // 锁到期时间
