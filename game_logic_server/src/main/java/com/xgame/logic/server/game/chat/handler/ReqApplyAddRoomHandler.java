...
public class ReqApplyAddRoomHandler extends PlayerCommand<ReqApplyAddRoomMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.applyAddRoom(crossPlayer, message.roomKey);
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.applyAddRoom(crossPlayer, message.roomKey);
			}
		}
    }
}
