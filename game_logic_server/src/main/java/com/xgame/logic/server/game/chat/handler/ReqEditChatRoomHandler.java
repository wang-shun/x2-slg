...
public class ReqEditChatRoomHandler extends PlayerCommand<ReqEditChatRoomMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.editRoomInfo(crossPlayer, Long.valueOf(message.roomKey), message.roomName, message.desc, message.open);
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.editRoomInfo(crossPlayer, Long.valueOf(message.roomKey), message.roomName, message.desc, message.open);
			}
		}
    }
}
