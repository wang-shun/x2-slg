...
public class ReqDealRoomApplyHandler extends PlayerCommand<ReqDealRoomApplyMessage>{
    ...
    public void action() {
		...
		if(serverId == InjectorUtil.getInjector().serverId) {
			chatRoomManager.dealApply(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId, message.result);
		} else {
			RemoteService remoteService = crossManager.getRemoteService(serverId);
			if(remoteService != null) {
				remoteService.dealApply(crossPlayer, Long.valueOf(message.roomKey), message.targetPlayerId, message.result);	
			}
		}
    }
}
